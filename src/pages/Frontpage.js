...
const Frontpage = () => {
  ...
  return (
    <div className="container">
      <h1 style={{ textAlign: "center" }}>{t("TITLE.HEADER")}</h1>
      <div className="search">
        <input
          ...
          style={{ width: "300px" }}
        />
        <button onClick={fetchServiceById}>{t("BUTTON.SEARCH")}</button>
      </div>
      <br />
      <Space size="middle" style={{ margin: 10 }}>
        <button>{t("BUTTON.REFRESH")}</button>

        <button onClick={addNewService}>{t("BUTTON.ADD")}</button>
      </Space>
      <br />
      <br />
        ...
        {services.map((service) => (
          <div key={service.id} className="card">
            <div className="service-text">
              ...
              <p>
                <b>{t("SERVICES.SERVICE")}</b> {t(`${service.type}`)}
              </p>
              <p>
                <b>{t("SERVICES.START")}</b> {service.start}
              </p>
              <p>
                <b>{t("SERVICES.DONE")}</b> {service.done}
              </p>
              <p>
                <b>{t("SERVICES.CUSTOMER")}</b> {service.customer}
              </p>
              <p>
                <b>{t("SERVICES.COUNTRY")}</b> {t(`${service.country}`)}
              </p>
              <p>
                <b>{t("SERVICES.CITY")}</b> {t(`${service.city}`)}
              </p>
              <p>
